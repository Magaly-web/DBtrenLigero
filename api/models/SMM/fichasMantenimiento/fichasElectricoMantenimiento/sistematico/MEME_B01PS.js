...
var memeB01PS = require('../../../../../models/SMM/fichasMantenimiento/fichasElectricoMantenimiento/sistematico/MEME_B01PS');
const MEME_B01PS = {};

MEME_B01PS.getMEME_B01PSData = async (req, res) => {
    const {id} = req.params;

    const maintenance = await memeB01PS.findById(id)
    res.json(maintenance)
};
MEME_B01PS.editMEME_B01PSData = async (req, res) => {
    const { id } = req.params;
    let materiales = {
    ...
    await memeB01PS.findByIdAndUpdate(id, { $set: MEME_B01PS}, { new: true });
    res.json({
        status: 'Employee update'
    });
}
MEME_B01PS.checkedApprovalByWorker = async (req, res) => {
    const { id } = req.params;
    const checked = {
        documentFormCurrentState: req.body
    }
    await memeB01PS.findByIdAndUpdate(id, { $set: checked }, { new: true });
    res.json({
        status: 'worker state update'
    })
}
MEME_B01PS.getStateCheckedApprovalByWorker = async (req, res) => {
    const { id } = req.params;
    const state = await memeB01PS.findById(id, {
        "_id": 1, 
        "documentFormCurrentState": 1
    })
    res.json(state)
}
MEME_B01PS.getAllMaintenenceMEME_B01PS = async (req, res) => {
    const maintenances = await memeB01PS.find({}, {
        "_id": 1, 
        "unidad": 1, 
...
MEME_B01PS.addNewHistoryrecord = async (req, res) => {
    const { id } = req.params;
    const historyMEME_B01PSNew = {
        historyFile: req.body.historyFile || []
    }
    await memeB01PS.findByIdAndUpdate(id, { $set: historyMEME_B01PSNew }, { new: true });
    res.json({
        status: 'History update'
    })
}
MEME_B01PS.getHistoryOF = async (req, res) => {
    const { id } = req.params;
    const maintenances = await memeB01PS.findById(id, {
        "_id": 0, 
        "historyFile": 1
    })
    res.json(maintenances)
}
MEME_B01PS.createMaintenenceMEME_B01PS = async (req, res) => {
    const maintenence = new memeB01PS(req.body)
    await maintenence.save()
    res.json({res: 'Ok'})
};
module.exports = MEME_B01PS;