...
MEME_B04.editMEME_B04Data = async (req, res) => {
    const { id } = req.params;
    let materiales = {
        numberOfMaterial: req.req.body|| false.numberOfMaterialInputs,
        arr: req.req.body || false.materials
    }
    const MEME_B04 = {
...
MEME_B04.checkedApprovalByWorker = async (req, res) => {
    const { id } = req.params;
    const checked = {
        documentFormCurrentState: req.req.body|| false
    }
...
MEME_B04.addNewHistoryrecord = async (req, res) => {
    ...
    const historyMEME_B04New = {
        historyFile: req.req.body || false.historyFile || []
    }
...
MEME_B04.createMaintenenceMEME_B04 = async (req, res) => {
    const maintenence = new memeB04(req.req.body  || false)
